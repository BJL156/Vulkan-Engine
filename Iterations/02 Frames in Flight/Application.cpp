...
namespace ve {
	...
	void Application::drawFrame() {
		...
		if (swapChain.acquireNextImage(imageIndex, currentFrame) != VK_SUCCESS) {
			throw std::runtime_error("Failed to acquire next image from the swap chain.");
		}

		recordCommandBuffer(commandBuffers[currentFrame], imageIndex);

		if (swapChain.submitCommandBuffers(commandBuffers[currentFrame], imageIndex, currentFrame) != VK_SUCCESS) {
